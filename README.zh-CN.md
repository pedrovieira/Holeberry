...
- **自动 gravity 更新** —— 按计划定时更新 Pi-hole 的 gravity（每 6/12 小时、每天、每周，或永不更新）。
...
### 为什么我的浏览器标签页没有出现？
需要在「设置 → 通用」中启用浏览器标签页解除屏蔽，并且 Holeberry 需要获得浏览器的自动化权限（首次使用时在系统设置中授予）。在 macOS 27 及更高版本中，读取 Firefox/Zen/Waterfox 还需要在「系统设置 → 隐私与安全性 → 文件与文件夹」中为 Holeberry 开启对应浏览器的一次性授权；访问被阻止时，Holeberry 会提供该设置的直达链接。
