...
Holeberry は Developer ID で署名され、Apple による完全な公証（notarization）を受けています。
...
ウェブ管理画面を開かずに Pi-hole のブロックリストを最新に保てます：メニューバーから直接 **gravity 更新**を実行でき、**すべてのサーバーに同時に**適用されます。メインメニューから実行するか、設定で登録したグローバルキーボードショートカットで実行できます。

- **自動 gravity 更新** —— スケジュールに従って Pi-hole の gravity を更新します（6/12 時間ごと、毎日、毎週、または更新しない）。
...
使えます。パスワード未設定のインスタンス（v5・v6 とも）を完全にサポートしています——接続画面でパスワード欄を空のままにしてください。そのようなインスタンスには認証情報は一切保存されません。
...
いいえ——置き換えるものではありません。Holeberry は日常の操作（ステータス確認、オン/オフ切り替え、ピンポイントのブロック解除）のリモコンです。詳細な設定（adlist、DHCP など）には引き続きウェブインターフェースを使います——ただし、gravity の更新はメニューバーから直接実行できます。

### ブラウザタブが表示されないのはなぜですか？
「設定 → 一般」でブラウザタブのブロック解除を有効にし、Holeberry にブラウザの Automation 権限が必要です（初回使用時にシステム設定で許可）。macOS 27 以降では、Firefox／Zen／Waterfox の読み取りに「システム設定 → プライバシーとセキュリティ → ファイルとフォルダ」で Holeberry に該当ブラウザを許可する一度だけの設定も必要です。アクセスがブロックされている場合、Holeberry がこの設定へのリンクを表示します。
