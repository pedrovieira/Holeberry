...
ウェブ管理画面を開かずに Pi-hole のブロックリストを最新に保てます：メニューバーから直接 **gravity 更新**を実行でき、**すべてのサーバーに同時に**適用されます。メインメニューから実行するか、設定で登録したグローバルキーボードショートカットで実行できます。メニューには gravity が最後に更新された時刻が表示され、完了時には通知でお知らせします。

Gravity 更新を実行できるのは Pi-hole® **v6** インスタンスのみです——v5 の API には提供されていません。v5 のみの構成でも、Holeberry は最終更新時刻を表示しますが、更新の実行にはウェブ管理画面が必要です。
